...
#	time.sleep(5)
	...
	flagin = 0
	try:
		webdriver.support.wait.WebDriverWait(driver,40).until(webdriver.support.expected_conditions.text_to_be_present_in_element((By.CLASS_NAME,'msg2'),'搜索结束'))
		flagex = 1
		f1.write(dep+' '+arv+'\n')
	except Exception as e:
		f2.flush()
	try:	
		webdriver.support.wait.WebDriverWait(driver,50).until(webdriver.support.expected_conditions.text_to_be_present_in_element((By.CLASS_NAME,'dec'),'搜索结束'))
		flagin = 1
		f2.write(dep+' '+arv+'\n')
	except Exception as e:
		f1.flush()
