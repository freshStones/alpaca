...
while  a == 1:
	try:
		...
		fl = result.find_elements_by_class_name('c1')
		tm = result.find_elements_by_class_name('c2')
		lacal = result.find_elements_by_class_name('c3')
		c6 = result.find_elements_by_class_name('c6')
		conn = MySQLdb.connect(host='162.105.30.55',user='remote',passwd='alpaca',db='airTicketOnline',port=3306,charset='utf8')
		cur = conn.cursor()
		print type(fl)
		for i in range(0,len(fl)):
			fl_name = fl[i].find_element_by_class_name('a_name').text
			fl_model = fl[i].find_element_by_class_name('a_model').text
			tm_dep = tm[i].find_element_by_class_name('a_tm_dep').text
			tm_arv = tm[i].find_element_by_class_name('a_tm_arv').text
			lacal_dep = lacal[i].find_element_by_class_name('a_lacal_dep').text
			lacal_arv = lacal[i].find_element_by_class_name('a_local_arv').text
			prc = c6[i].find_element_by_class_name('prc').text
			...
			value = (fl_name,fl_model,tm_dep,tm_arv,lacal_dep,lacal_arv,prc,time.mktime(today.timetuple()))
			cur.execute(query,value);
			conn.commit()
			print fl_name+fl_model+tm_dep+tm_arv+lacal_dep+lacal_arv+prc
